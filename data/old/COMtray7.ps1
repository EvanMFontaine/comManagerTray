...
# Function to open COM port with PlatformIO monitor
    ...
    if ($ports.Count -eq 0) {
        ...
        foreach ($port in $ports) {
            ...
            if ($port.DeviceInfo) {
                ...
                # Extract USB info from device instance path
                $deviceID = $port.DeviceInfo.DeviceID
                $hardwareID = (Get-PnpDeviceProperty -InstanceId $port.DeviceInfo.DeviceID -KeyName "DEVPKEY_Device_HardwareIds" -ErrorAction SilentlyContinue).Data | Select-Object -First 1
                ...
                if ($hardwareID -match 'VID_([0-9A-F]+)&PID_([0-9A-F]+)') {
                    ...
                    $usbInfoItem.Text = "USB Info: VID_$_vid PID_$_pid"
                    $usbInfoItem.Enabled = $false
                    $portItem.DropDownItems.Add($usbInfoItem)
                }
                
                # Add manufacturer if available
                $manufacturer = (Get-PnpDeviceProperty -InstanceId $port.DeviceInfo.DeviceID -KeyName "DEVPKEY_Device_Manufacturer" -ErrorAction SilentlyContinue).Data
                if ($manufacturer) {
                    $mfgItem = New-Object System.Windows.Forms.ToolStripMenuItem
                    $mfgItem.Text = "Manufacturer: $manufacturer"
                    $mfgItem.Enabled = $false
                    $portItem.DropDownItems.Add($mfgItem)
                }
            }
