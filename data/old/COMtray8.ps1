...
function Open-COMPort {
    ...
    $cmbBaud.Items.AddRange(@("9600", "19200", "38400", "57600", "115200", "230400", "460800", "500000", "576000", "921600", "1000000", "1152000", "1500000", "2000000", "2500000", "3000000", "3500000", "4000000", "6000000"))
    $cmbBaud.Text = "115200"
    ...
    if ($ports.Count -eq 0) {
        ...
        foreach ($port in $ports) {
            ...
            if (-not $port.InUse) {
                ...
                $portItem.DropDownItems.Add($openItem)
                
                $portItem.DropDownItems.Add((New-Object System.Windows.Forms.ToolStripSeparator))
            }
            ...
            # Add USB info loading on demand
            $portItem.DropDownOpening = {
                param($_sender, $e)
                
                # Check if USB info already loaded
                if ($_sender.DropDownItems.Count -le 3) {  # Only has basic items
                    $portName = $_sender.DropDownItems[0].Tag  # Get port name from first item
                    
                    if ($portName) {
                        $usbInfo = Get-USBInfo -portName $portName
                        
                        if ($usbInfo) {
                            $_sender.DropDownItems.Add((New-Object System.Windows.Forms.ToolStripSeparator))
                            
                            # Update main item text with description
                            $_sender.Text = $usbInfo.Description
                            
                            if ($usbInfo.VID -and $usbInfo.PID) {
                                $usbInfoItem = New-Object System.Windows.Forms.ToolStripMenuItem
                                $usbInfoItem.Text = "USB Info: VID_$($usbInfo.VID) PID_$($usbInfo.PID)"
                                $usbInfoItem.Enabled = $false
                                $_sender.DropDownItems.Add($usbInfoItem)
                            }
                            
                            if ($usbInfo.Manufacturer) {
                                $mfgItem = New-Object System.Windows.Forms.ToolStripMenuItem
                                $mfgItem.Text = "Manufacturer: $($usbInfo.Manufacturer)"
                                $mfgItem.Enabled = $false
                                $_sender.DropDownItems.Add($mfgItem)
                            }
                        }
                    }
                }
            ...
            $portItem.Add_DropDownOpening($portItem.DropDownOpening)
