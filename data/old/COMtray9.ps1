...
function Update-Menu {
    ...
    if ($ports.Count -eq 0) {
        ...
        foreach ($port in $ports) {
            ...
            # Get device description (quick lookup)
            $portInfo = Get-WmiObject Win32_PnPEntity -Filter "Name LIKE '%$($port.Name)%'" | Select-Object -First 1
            if ($portInfo) {
                $deviceName = $portInfo.Name
                if ($deviceName -match "\($($port.Name)\)") {
                    $deviceName = $deviceName -replace "\s*\($($port.Name)\)\s*", " "
                    $displayText = "$($port.Name) - $($deviceName.Trim())"
                } else {
                    $displayText = "$($port.Name) - $deviceName"
                }
                if ($displayText.Length -gt 60) {
            ...
            # Add USB info if it's a USB device (fast check)
            if ($portInfo) {
                try {
                    $hardwareID = (Get-PnpDeviceProperty -InstanceId $portInfo.DeviceID -KeyName "DEVPKEY_Device_HardwareIds" -ErrorAction SilentlyContinue).Data | Select-Object -First 1
                    ...
                    if ($hardwareID -match 'VID_([0-9A-F]+)&PID_([0-9A-F]+)') {
                        ...
                        $portItem.DropDownItems.Add($usbInfoItem) | Out-Null
                        
                        # Get manufacturer (only if we have USB info)
                        $manufacturer = (Get-PnpDeviceProperty -InstanceId $portInfo.DeviceID -KeyName "DEVPKEY_Device_Manufacturer" -ErrorAction SilentlyContinue).Data
                        if ($manufacturer) {
                            $mfgItem = New-Object System.Windows.Forms.ToolStripMenuItem
                            $mfgItem.Text = "Manufacturer: $manufacturer"
                            $mfgItem.Enabled = $false
                            $portItem.DropDownItems.Add($mfgItem) | Out-Null
                        }
                    }
                } catch {}
            }
